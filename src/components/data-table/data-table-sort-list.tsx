...
import type { ExtendedColumnSort, ExtendedSortingState, StringKeyOf } from "@/types/data-table.ts";
import { type SortDirection, type Table } from "@tanstack/react-table";
...
export function DataTableSortList<TData>({ table, debounceMs }: DataTableSortListProps<TData>) {
  ...
  const sortableColumns = React.useMemo(
    () =>
      table
        .getAllColumns()
        .filter((column) => column.getCanSort() && !sorting.some((s) => s.id === column.id))
        .map((column) => ({
          id: column.id,
      ...
      >
        {uniqueSorting.length > 0 ? (
          ...
          <div className="flex flex-col gap-2">
            <h4 className="font-medium leading-none">Chưa có sắp xếp</h4>
            <p className="text-sm text-muted-foreground">Hãy sắp xếp để kết quả gọn gàng hơn</p>
          </div>
        )}
        <div className="flex max-h-40 flex-col gap-2 overflow-y-auto p-0.5">
          <Sortable value={sorting} onValueChange={setSorting}>
            {uniqueSorting.map((sort: any) => {
              ...
              const columnLabel = column?.columnDef.meta?.label ?? toSentenceCase(sort.id);
        ...
        <div className="flex w-full items-center gap-2">
          ...
          {sorting.length > 0 ? (
            <Button
              ...
              variant="outline"
              className="rounded"
              onClick={() => table.setSorting(initialSorting)}
            >
              Đặt lại
            </Button>
          ) : null}
