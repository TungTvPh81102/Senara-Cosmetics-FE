...
import type { DataTableAdvancedFilterField, Filter, StringKeyOf } from "@/types/data-table";
        ...
        return (
          <div className="flex items-center gap-2">
            <Input
              ...
              className="h-8 w-full rounded"
              defaultValue={typeof filter.value[0] === "string" ? filter.value[0] : undefined}
              onChange={(event) =>
                updateFilter({
                  rowId: filter.rowId,
            ...
            <Input
              ...
              className="h-8 w-full rounded"
              defaultValue={typeof filter.value[1] === "string" ? filter.value[1] : undefined}
              onChange={(event) => {
                updateFilter({
                  rowId: filter.rowId,
          ...
          <Popover>
            ...
            <PopoverContent id={`${inputId}-calendar`} align="start" className="w-auto p-0">
              <Calendar
                ...
                initialFocus
                numberOfMonths={2}
                defaultMonth={dateValue[0] ? new Date(dateValue[0]) : new Date()}
              />
            </PopoverContent>
          </Popover>
        );
      ...
      >
        {filters.length > 0 ? (
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-sm text-muted-foreground">Thêm bộ lọc để lọc kết quả</p>
          </div>
        )}
                  ...
                  >
                    <Command>
                      ...
                      <CommandList>
                        ...
                        <CommandGroup>
                          {filterFields
                            .filter((f) => !filters.find((fil) => fil.id === f.id))
                            .map((field) => (
                              <CommandItem
                                key={field.id}
                                value={field.id}
                              ...
                              >
                                <span className="mr-1.5 truncate">{field.label}</span>
                                <Check
                                  className={cn(
